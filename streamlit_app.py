...
# [v8.8] 출석부 업로드 및 전도사별 설정칸 완전 복구 버전
...
class FinalEngineV88:
    ...
    def generate_unique_report(name, adm, text, sit, strat):
        ...
        if len(neg_hits) > len(pos_hits):
            psy_status = f"현재 {name} 님은 신앙적 성장보다는 외부적 환경 요인({', '.join(set(neg_hits[:2]))})에 의한 심리적 간섭이 강하게 작용하고 있습니다."
        elif len(pos_hits) > 0:
            psy_status = f"{name} 님은 {curr_step} 과정의 말씀을 자기화하려는 의지가 상담 기록 상의 '{pos_hits[0]}' 등의 표현에서 드러납니다."
        else:
            psy_status = f"특별한 감정 기복은 보이지 않으나, {curr_step} 단계의 핵심 목표에 대한 피드백이 부족하여 밀착 확인이 필요합니다."
        ...
        report += f"- {psy_status}\n"
        report += f"- **핵심 리스크:** 상담 일지 내 부정 키워드 감지 빈도가 {len(neg_hits)}회입니다.\n\n"
        ...
        report += f"- **에너지 운용:** {adm['ennea']} 에너지를 투입하여 대응 전략인 '{strat[:15]}...'을 개인 고민과 연결하십시오.\n\n"
        report += f"### 🚩 최종 분석 결론\n"
        report += f"- **위기 지수:** {risk_score}점\n"
        report += f"- **조치:** {'[긴급] 즉각적인 환경 차단 및 상급자 면담 요망' if risk_score > 75 else '정기 리포트를 통한 점진적 확신 심기 유지'}"
        
        return report, risk_score

# --- UI 설정 (사이드바에 파일 업로드칸 배치) ---
with st.sidebar:
    st.header("⚙️ 전략 엔진 v8.8")
    admins = []
    # 전도사별 설정 및 파일 업로드 칸 (A, B, C반)
    for t in ["A", "B", "C"]:
        with st.expander(f"👤 {t}전도사 설정 및 출석부 업로드"):
            f = st.file_uploader(f"{t}반 출석부 파일 (xlsx)", type=["xlsx"], key=f"v88_f_{t}")
...
with l:
    ...
    target = st.text_input("분석할 수강생 이름") if mode == "개인 상황 및 전략" else ""
    sit_in = st.text_area("🌐 현재 발생 상황 (Status)", height=100)
    strat_in = st.text_area("🛡️ 수립 대응 전략 (Strategy)", height=100)
    
    if st.button("데이터 정밀 분석 실행", use_container_width=True):
        active = [a for a in admins if a['file']]
        if not active: 
            st.error("사이드바에서 최소 하나 이상의 출석부 파일을 업로드해 주세요.")
        else:
            final_res = []
            for adm in active:
...
if 'v88_final' in st.session_state:
    ...
    with r:
        if not data:
            st.warning("입력하신 이름과 일치하는 수강생 데이터를 찾을 수 없습니다.")
            ...
            for item in data:
                with st.expander(f"➔ {item['name']} 분석 결과 (위험도: {item['risk']}점)"):
